private ["_marker","_location","_object","_markers","_wreck2","_wreck3","_wreck4","_wreck5","_vehicles"];
...
_object = "LADAWreck" createvehicle _location;
...
_object setVehicleInit "i=[this,4,time,false,false] spawn core_burn"; processInitCommands;

_wreck2 setPos (_object modelToWorld [4.5,4.5,0]);
_pos = getPos _wreck2;
_wreck2 setPos [_pos select 0,_pos select 1,0];
_wreck3 setPos (_object modelToWorld [-4.5,-6,0]);
_pos = getPos _wreck3;
_wreck3 setPos [_pos select 0,_pos select 1,0];
_wreck4 setPos (_object modelToWorld [0,10,0]);
_pos = getPos _wreck4;
_wreck4 setPos [_pos select 0,_pos select 1,0];
_wreck5 setPos (_object modelToWorld [4.5,-6.5,0]);
_pos = getPos _wreck5;
...
_vehicles = [_object,_wreck2,_wreck3,_wreck4,_wreck5];

["ALL",[_object,['','noscript.sqf',format["[%1,%2] spawn accidents_clear",_vehicles,_markers],25,false,true,'LeanRight','player distance _target < 5 && {!([_target,"Clean up accident(E)","\crp_data\images\tags\jaws"]call tag_show)}']],"network_addAction",false,false]call network_MPExec;
